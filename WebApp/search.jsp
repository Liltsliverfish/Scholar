	...
	<head>
		...
		<c:choose>
			...
			<c:when test="${sessionScope.type.equals('Lesson')}">
				<title>教学平台 - 搜索课程 - "${sessionScope.searchString}"</title>
			</c:when>
			<c:when test="${sessionScope.type.equals('User')}">
				<title>教学平台 - 搜索用户 - "${sessionScope.searchString}"</title>
			</c:when>
		</c:choose>
			...
			<div class="content">
				...
				<form class="form-group" action="SearchSer" method="post">
					<div class="input-group col-xs-12">
						...
						<input type="text" name="searchString" id="searchString" class="form-control" 0="请您输入关键词">
